...
StatusOr<TFetchDataResultPtr> StatisticResultWriter::_process_chunk(Chunk* chunk) {
    ...
    if (version == STATISTIC_DATA_VERSION1) {
        ...
        RETURN_IF_ERROR_WITH_WARN(_fill_datacache_copilot_statistic(version, result_columns, chunk, result.get()), "Fill datacache copilot statistic data failed");
    }
    return result;
}
...
Status StatisticResultWriter::_fill_datacache_copilot_statistic(int version, const Columns& columns,
                                                                 const Chunk* chunk, TFetchDataResult* result) {
    SCOPED_TIMER(_serialize_timer);
    ...
    auto column_name = ColumnViewer<TYPE_VARCHAR>(columns[5]);
