...
public class FrequencyStatisticsStorage {
    private static final FrequencyStatisticsStorage FREQUENCY_STATISTICS_STORAGE = new FrequencyStatisticsStorage();

    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final CatalogMapping catalogMapping = new CatalogMapping();
    private long estimateMemorySize = 0L;

    public static FrequencyStatisticsStorage getInstance() {
        return FREQUENCY_STATISTICS_STORAGE;
    }

    public void addAccessItems(List<AccessItem> accessItems) {
        writeLock();
    ...
    public Optional<String> exportInsertSQL(String catalog, String database, String table) {
        writeLock();
        try {
            ...
            sb.append(String.format("INSERT INTO `%s`.`%s`.`%s` VALUES ", catalog, database, table));
                            ...
                            for (Map.Entry<String, AccessTimeMapping> columnEntry : columnMapping.mapping.entrySet()) {
                                ...
                                for (Map.Entry<Long, Long> accessTimeEntry : accessTimeMapping.mapping.entrySet()) {
                                    ...
                                    values.add(String.format(
                                            "('%s', '%s', '%s', '%s', '%s', from_unixtime(%d, 'yyyy-MM-dd HH:mm:ss'), %d)",
    ...
    private class CatalogMapping {
        ...
        protected void update(AccessItem accessItem) {
    ...
    private class AccessTimeMapping {
        ...
        protected void update(AccessItem accessItem) {
            long accessTime = accessItem.getAccessTimeSecs();
